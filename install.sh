...
echo "=================================================="
echo "🚀 开始安装 阿里云CDT保活流量控制 & TG 机器人系统..."
echo "=================================================="

# 1. 创建工作目录并进入
WORKDIR="/root/aliyun-traffic-control"
mkdir -p $WORKDIR
cd $WORKDIR
...
if [ -f /etc/debian_version ]; then
    apt-get update && apt-get install -p python3-venv python3-pip curl -y
elif [ -f /etc/redhat-release ]; then
    yum install python3 python3-pip curl -y
fi

# 3. 下载 GitHub 仓库中的核心文件 (请将下面的 URL 替换为你自己的 GitHub 地址)
GITHUB_USER="你的GitHub用户名"
REPO="aliyun-traffic-control"
...
# 5. 配置 Systemd 服务并开机自启
echo "🤖 正在启动 Telegram 机器人常驻后台服务..."
systemctl daemon-reload
systemctl enable traffic-control.service
systemctl restart traffic-control.service

# 6. 配置 Crontab 每分钟定时保活
echo "⏱️  正在配置 Crontab 每分钟静默保活..."
# 检查是否已经存在该任务，避免重复添加
CRON_CMD="* * * * * /root/aliyun-traffic-control/venv/bin/python /root/aliyun-traffic-control/traffic_control.py >> /root/aliyun-traffic-control/cron_run.log 2>&1"
(crontab -l 2>/dev/null | grep -Fv "traffic_control.py"; echo "$CRON_CMD") | crontab -

echo "=================================================="
echo "🎉 安装完成！"
echo "🤖 TG 机器人已在后台常驻 (包含早9点推送)"
echo "🛡️ 纯后台每分钟保活已挂载至 Crontab"
echo "📊 你可以运行 tail -f $WORKDIR/cron_run.log 查看保活日志"
echo "=================================================="