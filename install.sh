...
# ✨ 终极修复：在切换目录前，先用绝对路径牢牢记住当前运行的脚本位置
REAL_SCRIPT_PATH=$(realPath "$0" 2>/dev/null || echo "$(pwd)/$0")
...
# ✨ 核心改进：在切目录前，先把自身安全地备份过去，规避找不到文件的Bug
cp "$REAL_SCRIPT_PATH" "$WORKDIR/install.sh" 2>/dev/null || true
