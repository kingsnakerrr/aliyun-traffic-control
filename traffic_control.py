...
REGION_ID = 'cn-hongkong'      # ✨ 完美保留并支持动态写入你的实例地域（如 cn-hongkong）

BILL_REG_ID = 'cn-hangzhou'    # 账单结算网关：默认国内杭州，国际版会被改为 ap-southeast-1
IS_INTERNATIONAL = False       # 开关：True 为国际版，False 为国内版
MAX_TRAFFIC_GB = 180.0         # 🛑 你的硬核流控阈值：保持 180G 不变

# 初始化阿里云客户端（实例操作使用各自的 REGION_ID，账单查询使用 BILL_REG_ID）
...
def start_vps_safety():
    ...
    if not INSTANCE_ID or '你的' in INSTANCE_ID:
        logging.error("❌ 自动开机失败：未配置有效的 INSTANCE_ID")
        return
    try:
        ...
        logging.error(f"自动开机失败，请检查密钥是否正确并拥有管理ECS权限: {str(e)}")

def stop_vps_safety():
    """触发熔断：强行关闭服务器"""
    if not INSTANCE_ID or '你的' in INSTANCE_ID:
        return "❌ 触发流控关机失败：未配置有效的 INSTANCE_ID"
    try:
        ...
        return f"🛑 已成功向实例 {INSTANCE_ID} 执行官方 API 强行关机"
    except Exception as e:
