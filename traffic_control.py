...
def get_billing_cycle():
    """全自动动态获取当前月份，格式化为 YYYY-MM (解决 MissingBillingCycle 问题)"""
    return datetime.datetime.now().strftime('%Y-%m')

def get_vps_status():
    """实时查询当前 ECS 实例的运行状态(Running/Stopped等)"""
...
def main():
    ...
    # 彻底解决异常导致不响应的问题，加入超时重试容错
    updater = Updater(TELEGRAM_BOT_TOKEN, request_kwargs={'read_timeout': 6, 'connect_timeout': 7})
